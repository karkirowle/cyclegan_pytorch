...
class VCC2016DataSource(VCC2016Super):
    ...
    def __init__(self, data_root, speakers, training):
        ...
        self.preprocess_dir = "./preprocessed/VCC2016"
    ...
    def collect_features(self, file_path):
        ...
        save_path = os.path.join(
            self.preprocess_dir, self.speaker, os.path.basename(file_path))
...
class MCEPWrapper(Dataset):
    ...
    def __init__(self, input_file_source, output_file_source, mfcc_only, num_frames=128, norm_calc=True, randomize=True, dtw=True):
    ...
    def __getitem__(self, idx):
        ...
        if self.randomize:
            ...
            input_slice = input_temp[:, :]
            output_slice = output_temp[:, :]
            input_mean, input_std = self.input_meanstd
            output_mean, output_std = self.output_meanstd

        mcep_A_normalised = (
            input_slice - input_mean[1:25])/input_std[1:25]
        mcep_B_normalised = (
            output_slice - output_mean[1:25])/output_std[1:25]
...
if __name__ == '__main__':
    ...
    data_source = VCC2016DataSource(
        "/home/boomkin/repos/Voice_Converter_CycleGAN/data", ["SF1"])
