...
class _ConvLayer(nn.Module):
    ...
    def __init__(self, in_filter, out_filter, kernel, stride):
        ...
        if type(stride) == tuple:
            ...
            self.cut_last_element_X = (
                self.kernel[0] % 2 == 0 and self.stride[0] == 1)
            self.cut_last_element_Y = (
                self.kernel[1] % 2 == 0 and self.stride[1] == 1)
...
class ConvLayer1D(_ConvLayer):
    ...
    def __init__(self, in_filter, out_filter, kernel, stride):
        ...
        self.conv = nn.Conv1d(self.in_filter, self.out_filter,
                              self.kernel, stride, padding=self.padding)
...
class ConvLayer2D(_ConvLayer):
    ...
    def __init__(self, in_filter, out_filter, kernel, stride):
        super().__init__(in_filter, out_filter, kernel, stride)
        self.conv = nn.Conv2d(self.in_filter, self.out_filter,
                              self.kernel, stride, padding=self.padding)
...
class Downsample1D(Downsample):
    ...
    def __init__(self, in_filter, out_filter, kernel, stride):
        ...
        self.instance_norm = nn.InstanceNorm1d(
            num_features=out_filter*2, eps=1e-6, affine=True)
...
class Downsample2D(Downsample):
    ...
    def __init__(self, in_filter, out_filter, kernel, stride, padding=None):
        ...
        if padding is not None:
            self.conv.conv.padding = padding
...
class Generator(nn.Module):
    ...
    def __init__(self, in_feature):
        ...
        self.block = nn.Sequential(ConvLayer1D(in_filter=in_feature, out_filter=128 * 2, kernel=15, stride=1),
                                   ...
                                   Downsample1D(
                                       in_filter=128, out_filter=256, kernel=5, stride=2),
                                   Downsample1D(
                                       in_filter=256, out_filter=512, kernel=5, stride=2),

                                   ResidualBlock1D(
                                       in_filter=512, out_filter=512, kernel=3),
                                   ResidualBlock1D(
                                       in_filter=512, out_filter=512, kernel=3),
                                   ResidualBlock1D(
                                       in_filter=512, out_filter=512, kernel=3),
                                   ...
                                   ConvLayer1D(
                                       in_filter=128, out_filter=in_feature, kernel=5, stride=1),
                                   )
...
class Discriminator(nn.Module):
    ...
    def __init__(self, in_feature=1):
        ...
        self.block = nn.Sequential(
            ...
            ConvLayer2D(in_filter=in_feature, out_filter=256,
                        kernel=(3, 3), stride=(1, 2)),
            nn.GLU(dim=1),
            # input: (1 x 128 x 24 x 64)
            Downsample2D(in_filter=128, out_filter=256,
                         kernel=(3, 3), stride=(2, 2)),
            # input: (1 x 256 x 12 x 32)
            Downsample2D(in_filter=256, out_filter=512,
                         kernel=(3, 3), stride=(2, 2)),
            # input: (1 x 512 x 6 x 16)
            #nn.ZeroPad2d((3, 2, 0, 1)),
            Downsample2D(in_filter=512, out_filter=1024,
                         kernel=(6, 3), stride=(1, 2)),
